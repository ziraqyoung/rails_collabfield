...
RSpec.describe Post, type: :model do
  ...
  context 'Scopes' do
    ...
    it "by_category posts scope gets post by particular category" do
    ...
    it "search scope finds a marching post" do
      post = create(:post, title: "awesome title", content: 'great content' * 5)
      create_list(:post, 10, title: ('a'..'b' * 2).to_a.shuffle.join)
  ...
  context "Validations" do
    ...
    it "not valid with title shorter than 5 characters" do
      post.title = 'a' * 4
      expect(post).not_to be_valid
    end

    it "not valid with title more than 255 characters" do
      post.title = 'a' * 256
      expect(post).not_to be_valid
    end
