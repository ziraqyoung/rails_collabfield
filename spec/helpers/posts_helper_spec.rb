...
RSpec.describe PostsHelper, :type => :helper do
  ...
  context '#create_new_post_partial_path' do
    ...
    it "returns a non_signed_in partial_path" do
  ...
  context "#all_categories_button_partial_path" do
    ...
    it "returns an not_all_selected partial's path" do
  ...
  context "#no_posts_partial_path" do
    ...
    it "returns a no_post partial's path" do
      assign( :posts, [] )
      expect(helper.no_posts_partial_path).to( eq 'posts/branch/no_posts' )
    end

    it "returns an empty_partial path" do
      assign(:posts, [1] )
